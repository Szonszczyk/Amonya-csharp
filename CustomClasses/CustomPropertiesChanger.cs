...
)
{
    ...
    )
    {
        foreach (var (propertyName, newPropertyValue) in newProperties)
        {
            ...
            {
                logger.Error($"Incorrect property '{propertyName}' in {itemName} variant!");
                continue;
            }
            ...
            switch (originalProp.GetValue(copiedItem.Properties))
            {
                case double d:
                    ...
                    if (jsonPropertyValue.ValueKind == JsonValueKind.Number || jsonPropertyValue.ValueKind == JsonValueKind.String)
                        ...
                        logger.Error($"Value for '{itemName}' property '{propertyName}' is of incorrect type. Expected 'double/string', got '{newPropertyValue}'({jsonPropertyValue.ValueKind})!");
                    break;
                case int i:
                    if (config?.Changes?.Minimum != null)
                    {
                    ...
                    if (jsonPropertyValue.ValueKind == JsonValueKind.Number || jsonPropertyValue.ValueKind == JsonValueKind.String)
                        ...
                        logger.Error($"Value for '{itemName}' property '{propertyName}' is of incorrect type. Expected 'int/string', got '{newPropertyValue}'({jsonPropertyValue.ValueKind})!");
                    break;
                case bool b:
                    if (jsonPropertyValue.ValueKind == JsonValueKind.False || jsonPropertyValue.ValueKind == JsonValueKind.True)
                        ...
                        logger.Error($"Value for '{itemName}' property '{propertyName}' is of incorrect type. Expected 'bool', got '{newPropertyValue}'({jsonPropertyValue.ValueKind})!");
                    break;
                case string s:
                    if (jsonPropertyValue.ValueKind == JsonValueKind.String)
                        TrySetProperty(overrideProperties, propertyName, newPropertyValue);
                    else
                        logger.Error($"Value for '{itemName}' property '{propertyName}' is of incorrect type. Expected 'string', got '{newPropertyValue}'({jsonPropertyValue.ValueKind})!");
                    break;
                case MongoId id:
                    ...
                    if (jsonPropertyValue.ValueKind == JsonValueKind.String && itemTpl is not null)
                        ...
                        logger.Error($"Value for '{itemName}' property '{propertyName}' is of incorrect type. Expected 'MongoId', got '{newPropertyValue}'({jsonPropertyValue.ValueKind})!");
                    break;
                case HashSet<string> hSet:
                    if (jsonPropertyValue.ValueKind == JsonValueKind.Array)
                        TrySetProperty(overrideProperties, propertyName, newPropertyValue);
                    else
                        logger.Error($"Value for '{itemName}' property '{propertyName}' is of incorrect type. Expected 'array<string>', got '{newPropertyValue.ToString}'({jsonPropertyValue.ValueKind})!");
                    break;
                case null:
                    break;
                default:
