...
{
    private readonly HashSet<string> allVariantsAddedToQuests = [];
    ...
    private readonly HashSet<string> currencies = ["Roubles", "Dollars", "Euros", "GPCoins"];
    public int questsGenerated { get; private set; } = 0;
    ...
    public void GenerateQuests()
    {
        ...
        foreach (var (caliberName, questCategory) in questsConfig)
        {
            ...
            foreach (var (questName, quest) in questCategory.Quests)
            {
                ...
                if (diff is null)
                {
                    logger.Error($"Difficulty '{quest.Diff}' is missing in QuestConfig!");
                    continue;
                }
                ...
                var bulletsInQuest = new List<string>(quest.Unlocks);
                ...
                foreach (var unlock in bulletsInQuest)
                {
                    ...
                    {
                        if(!unlocks.TryAdd(bulletForUnlock.Id, bulletForUnlock))
                        {
                            logger.Error($"Duplicate bullet '{unlock}' in quest unlocks!");
                            continue;
                        }
                    ...
                    } else logger.Error($"Bullet '{unlock}' not found!");
                }
                    ...
                    switch (req)
                    {
                        ...
                        case "PMC":
                            ...
                            if (req != "AnyKills")
                            {
                                ...
                                {
                                    logger.Warning($"Kill conditions \"kill with weapon\" for quest {questName} can't find any weapons in database. Maybe \"{questCategory.Caliber.Id}\" caliber is incorrect?");
                                }
                        ...
                        case "Collector":
                            ...
                            break;
                        default:
                            if (!configLoader.QConfig.RequiresCategories.TryGetValue(req, out var category) || category.Count == 0)
                            {
                                logger.Error($"Category {req} not found in questConfig!");
                                continue;
                            }
    ...
    private string GenerateAssortForVariantBullet(string itemId, BulletsDatabase bullet, int loyalLevel = 1)
    {
        ...
        var traderAssort = TraderAssort ?? throw new InvalidOperationException("QuestGenerator.Initialize must run first.");
        traderAssort.Items ??= [];
    ...
    private Reward GenerateAssortUnlock(string questName, string itemId, BulletsDatabase bullet)
    {
        ...
        Reward reward = new()
        {
            ...
            TraderId = new StringOrInt(modDatabaseLoader.TraderBase.Id.ToString(), null),
    ...
    private void AddModdedBulletsToQuest(string questInternalId, Interfaces.QuestData questCategory)
    {
        ...
        {
            logger.Debug($"Quest Tree {questInternalId} does not have any valid unlocks or caliber is incorrect");
            return;
        }
        ...
        foreach (var bullet in moddedBullets)
        {
            ...
            if (ratings is null)
            {
                logger.Debug($"{questInternalId}: Can't determinate to what quest add {bulletDb.Name} as it is different category ({bulletDb.Category}) than the other bullets");
                continue;
            }
            var minRating = ratings.Min();
            var maxRating = ratings.Max();
            var ratingAdjusted = maxRating == minRating ? 0d : (bulletDb.Rating - minRating) / (maxRating - minRating);
            var questNumber = Math.Clamp((int)(ratingAdjusted * (maxQuests - 1)), 0, maxQuests - 1);
            logger.Debug($"{questInternalId}: Added {bulletDb.Name} bullet of {bulletDb.Category} category to {questNumber} quest");
